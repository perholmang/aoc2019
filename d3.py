...
def build_wires(x, y, dx=0, dy=0):
    ...
    if dy is not 0:
        ...
        for ay in range(y, y + dy + step, step):
            wires.append((x, ay))

    return wires


def closest_intersection(g1, g2):
    i = intersections(g1, g2)
    manhattans = list(
        filter(lambda x: x > 0, map(lambda x: manhattan_distance(x, (0, 0)), i))
    )
...
def build_wire_grid(str):
    wire_grid = []
    x = y = 0
    for step in str.split(","):
        m = re.search(r"([UDRL])(\d+)", step)
        direction, distance = m.group(1), int(m.group(2))
        dx, dy = DELTAS[direction]
        wire_grid.extend(build_wires(x, y, dy=dy * distance, dx=dx * distance))
        y += dy * distance
        x += dx * distance

    return list(dict.fromkeys(wire_grid))


def main():
    grid1 = build_wire_grid(
